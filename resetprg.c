...
//    IWDT(独立ウオッチドックタイマ)専用オンチップオシレータ動作 15 kHz
//
static void operating_frequency_set (void)
{
	...
	SYSTEM.ILOCOCR.BIT.ILCSTP = 0;	// IWDT(独立ウオッチドックタイマ)専用オンチップオシレータ動作
	delay_50usec();			// 50usec待つ IWDT専用クロック発振安定時間(tILOCO) (表39.24 クロックタイミングより)
